...
CHENTRY=$(echo -e "$TOGGLE\nmanual\n$LIST" | uniq -u | wofi -i -d --prompt "Wi-Fi SSID: " -H 300 -W 450)
#echo "$CHENTRY"
...
if [ "$CHENTRY" = "manual" ] ; then
	...
	MSSID=$(echo "enter the SSID of the network (SSID,password)" | wofi -d "Manual Entry: " -H 300 -W 450)
	...
	if [[ $(echo "$KNOWNCON" | grep "$CHSSID") = "$CHSSID" ]]; then
		...
		if [[ "$CHENTRY" =~ "WPA2" ]] || [[ "$CHENTRY" =~ "WEP" ]]; then
			WIFIPASS=$(echo "if connection is stored, hit enter" | wofi -P -d --prompt "password" -H 300 -W 450)
		fi
