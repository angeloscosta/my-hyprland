...
theme="wofi --style $DIR/wofi/lateral.css"
command="wofi $theme -d -L 7 -W 80 --define halign=center --define hide_search=true --define location=left -x 10"
...
case $chosen in
    ...
    $button_next)
        ...
        ;;
    $tog_repeat)
      mpc -q repeat && dunstify "$(mpc status | tail -n1 | awk -F '   ' '{print $2}')"
        ;;
    $tog_random)
      mpc -q random && dunstify "$(mpc status | tail -n1 | awk -F '   ' '{print $3}')"
        ;;
esac
